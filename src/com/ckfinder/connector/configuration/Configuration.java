...
public class Configuration implements IConfiguration {
		...
		if (node != null) {
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				if (childNode.getNodeName().equals("htmlExtensions")) {
					...
					Scanner scanner = new Scanner(htmlExt)
							.useDelimiter(",");
					while (scanner.hasNext()) {
						String val = scanner.next();
						if (val != null && !val.equals("")) {
							htmlExtensions.add(val.trim()
												.toLowerCase());
						}
	...
	private void setBasePathImpl(final String value) {
		try {
			...
			Class<IBasePathBuilder> clazz
								= (Class<IBasePathBuilder>) Class.forName(value);
	...
	private void setThumbs(final NodeList childNodes) {
		for (int i = 0; i < childNodes.getLength(); i++) {
			Node childNode = childNodes.item(i);
			if (childNode.getNodeName().equals("enabled")) {
				this.thumbsEnabled = Boolean
						.valueOf(childNode.getTextContent().trim());
			}
	...
	/**
	 * checks if reload config. Reloads it when date of file modifiation
	 * is other then date of last config initialization.
	 ...
	 * Empty method. It should be overrided if needed.
	 ...
	 * In every subclass this method should be override
	 * and return new instnce of it's class.
