...
public abstract class Command {
	 ...
	 * @return param value
	 */
	protected String getSessioin(final HttpServletRequest request,
									final String paramName) {
		if (request.getSession().getAttribute(paramName) == null) {
			return null;
		}
		return FileUtils.convertFromUriEncoding(
				request.getSession().getAttribute(paramName).toString(), configuration);
	}
	...
	protected void getCurrentFolderParam(final HttpServletRequest request) {
		...
		String currFolderSession = getSessioin(request, "currentFolder");
		if (currFolderSession !=null && !currFolderSession.equals("")) {
			this.currentFolder = PathUtils.addSlashToBeginning(PathUtils
					.addSlashToEnd(currFolderSession));
		}else {
