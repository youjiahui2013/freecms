			    ...
			    <tr>
			      ...
			      当前站点：<select id="site" onchange="siteChange()" name="site.id">
						<s:iterator value="siteList" status="bean">
							<option value="<s:property value="id"/>" ${site.id == id?"selected":"" }><s:property value="name"/>
						</s:iterator>
			      </select>
			      </td>
			      </tr>
				...
				<ul id="mixedpar" >
					...
					<s:if test="%{#session.loginAdmin.loginname == 'admin'}">
						<s:iterator value="channelList" status="bean">
							<li id="<s:property value="id"/>" class="hasChildren">
								<span><font size="2">
								<input  onclick="check('<s:property value="id"/>','<s:property value="name"/>',this)" ${haveChannelUser=='1'?'checked':'' } type="checkbox" name="sites" value="<s:property value="id"/>" show="<s:property value="name"/>"/>
					...
					<s:if test="%{#session.loginAdmin.loginname != 'admin'}">
						<s:iterator value="channelList" status="bean">
							<li id="<s:property value="id"/>" class="hasChildren">
								<span><font size="2">
								<input onclick="check('<s:property value="id"/>','<s:property value="name"/>',this)"  type="checkbox"  ${haveChannelUser=='1'?'checked':'' } name="sites" value="<s:property value="id"/>" show="<s:property value="name"/>"/>
								<b><s:property value="name"/></b></font>
								</span>
							</li>
						</s:iterator>
