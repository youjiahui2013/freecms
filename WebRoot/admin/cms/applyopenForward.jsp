...
<HTML>
	<HEAD>
		...
		<script type="text/javascript" src="js/applyopenForward.js"></script>

	</HEAD>
	<BODY>
		<FORM id=MyForm 
			method=post name=MyForm action=applyopen_forwardDo.do >
						...
						<TR>
							...
							<TD  align="left">
								<LABEL id=ctl01_ctl00_label>
										...
										<input type="hidden" name="applyopen.username" id="username" value="${bean.name }"/>
												</s:if>
										</s:iterator>
										<select id="userid" name="applyopen.userid" onchange="$('#username').val(this.options[this.selectedIndex].text);" >
