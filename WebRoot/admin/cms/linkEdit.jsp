				...
				<TABLE cellSpacing=4 cellPadding=4 width="95%" align=center>
					<TBODY>
							...
							<TD width="70%" align="left">
								<select id="linkSelect" name="link.parid">
									<s:iterator value="linkList" status="bean">
										<option value="<s:property value="id"/>" ${id eq linkClass.id ?"selected":"" } show="<s:property value="name"/>"><s:property value="name"/>
									</s:iterator>
								</select>
								<SPAN  id=ctl03>*</SPAN>
							</TD>
						...
						<TR style="display:${link.type=='2'?'block':'none' }">
							...
							<img src="${link.img }?date=<%=UUID.randomUUID() %>" width="60" height="40" title="点击查看大图"/>
							</a>
							</s:if>
							</span>
						...
						<TR>
							...
							<TD align="left">
								<input type="radio" name="link.isok" value="1" <s:if test="link==null || link.isok!=0">checked="checked"</s:if>>是
