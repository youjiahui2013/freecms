		...
		<s:if test="%{#session.loginAdmin.loginname == 'admin'}">
			<s:iterator value="siteList" status="bean">
				<li id="<s:property value="id"/>" class="hasChildren">
					<span><font size="2">
					<input onclick="siteCheck(this)" ${haveSiteUser=='1'?'checked':'' } type="checkbox" name="sites" value="<s:property value="id"/>" show="<s:property value="name"/>"/>
		...
		<s:if test="%{#session.loginAdmin.loginname != 'admin'}">
			<s:iterator value="siteList" status="bean">
				<s:if test="%{site.id != id}">
					<li id="<s:property value="id"/>" class="hasChildren">
						<span><font size="2">
						<input onclick="siteCheck(this)" type="checkbox"  ${haveSiteUser=='1'?'checked':'' } name="sites" value="<s:property value="id"/>" show="<s:property value="name"/>"/>
						<b><s:property value="name"/></b></font>
						</span>
					</li>
				</s:if>
