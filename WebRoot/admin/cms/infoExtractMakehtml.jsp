...
<HTML>
	...
	<BODY>
			...
			<DIV class="column">
				...
				<form action="info_extractMakehtml.do">
				<input type="hidden" name="pageFuncId" id="pageFuncId" value="${pageFuncId }"/>
			<input type="hidden" name="channel.id" value="${channel.id }"/>
				操作成功，请选择需要静态化的内容。<br/>
				<input type="checkbox" name="htmlChannel" value="1" checked/>目标栏目栏目页面静态化<br/>
				<input type="checkbox" name="htmlChannelPar" value="1"  checked/>目标栏目所有父栏目页面静态化<br/>
				<input type="checkbox" name="htmlIndex" value="1"  checked/>站点首页静态化<br/>
	
				<input type="submit" value="立即静态化"   class="button"  />&nbsp;
				<input type="button" value="不，我要继续提取信息" onclick="location.href='info_extract.do?pageFuncId=${pageFuncId }'"  class="button"  />
