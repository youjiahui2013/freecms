	...
	<ul id="mixedpar" >
		...
		<s:if test="%{#session.loginAdmin.loginname == 'admin'}">
			<s:iterator value="siteList" status="bean">
				<li id="<s:property value="id"/>" class="hasChildren">
					<span><font size="2">
					<a onclick="showDetail('<s:property value="id"/>','<s:property value="name"/>','')"><b><s:property value="name"/></b></a></font>
					</span>
					<ul>
						<li><span  >&nbsp;</span></li>
					</ul>
		...
		<s:if test="%{ #session.loginAdmin.loginname != 'admin'}">
			<s:iterator value="siteList" status="bean">
				<li id="<s:property value="id"/>" class="hasChildren">
					<span><font size="2">
					<a showDetail('<s:property value="id"/>','<s:property value="name"/>','')><b><s:property value="name"/></b></a></font>
					</span>
				</li>
			</s:iterator>
		</s:if>
