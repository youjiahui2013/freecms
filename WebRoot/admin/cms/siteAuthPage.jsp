		...
		<form action="site_authPageDo.do?site.id=${site.id }&role.id=${role.id }" method="post">
			<DIV class="column" style="width:99%">
				<div class="columntitle" style="padding-left:0px" title="选中则可以管理此站点">
					<b>${site.name }    管理站点权限</b><input type="checkbox"  id="manageSiteChecked" name="manageSiteChecked" ${roleSite!=null?"checked":"" } value="1"/>
				</div>
				...
				<table id="MyDataList" cellspacing="1" cellpadding="1"
					...
					<TR >
						<TD align="left">
						<input type="checkbox" onclick="siteadminCheck(this)" name="roleSite.siteadmin" ${"1" == roleSite.siteadmin?"checked":"" } value="1"/>
						站点总管理员(选中将可管理所有栏目)
						</TD>
					</TR>
					<TR id="channelTreeTd" style="display:${"1" == roleSite.siteadmin?"none":"block" }">
						...
						<ul id="mixed" >
							<li id="root" class="hasChildren">
								<span><font size="2"><input type="checkbox" onclick="checkAll(this.checked)" title="全选/取消全选"><b>栏目权限</b></font>
								</span>
								${channelTreeContent }
							</li>
						</ul>
